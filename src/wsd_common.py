...
log_path = os.environ.get("WSD_LOG_PATH", "/tmp/wsd-scan-log")
...
def soap_post_unicast(addr: str,
    ...
    try:
        ...
        while repeat:
            try:
                response = requests.post(addr, headers=headers, data=data, timeout=100)
                if wsd_globals.debug and (response.status_code >= 400 or len(response.content) == 0):
                    print("SOAP POST %s -> HTTP %s, %d byte(s), headers=%s" %
                          (addr, response.status_code, len(response.content), dict(response.headers)),
                          flush=True)
                    if response.content:
                        print(response.content[:1000], flush=True)
...
def submit_request(addrs: typing.Set[str],
    ...
    for addr in addrs:
        ...
        if r is None:
            continue
        if len(r) == 0:
            raise RuntimeError("Empty SOAP response for %s from %s" % (op_name, addr))
