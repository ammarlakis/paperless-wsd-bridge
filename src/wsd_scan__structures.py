...
class ScanTicket:
    ...
    def override_params(self, profile):
        ...
        if "images_to_transfer" in profile:
            self.doc_params.images_num = int(profile["images_to_transfer"])
        elif self.doc_params.input_src in ("ADF", "ADFDuplex", "Auto"):
            self.doc_params.images_num = 0
        else:
    ...
    def as_map(self):
        ...
        return {'JOB_NAME': self.job_name,
                ...
                'SIZE_AUTODETECT': xml_bool(self.doc_params.size_autodetect),
                'INPUT_W': self.doc_params.input_size[0],
                'INPUT_H': self.doc_params.input_size[1],
                'AUTO_EXPOSURE': xml_bool(self.doc_params.auto_exposure),
